...
const TaskTable = ({ tasks, handleFilterTasks, showActive }) => {
  ...
  return (
    <TableContainer component={Paper} sx={{ height: "80vh", border: "1px solid rgba(224, 224, 224, 1);"}}>
      <Table stickyHeader>
        ...
        <TableBody>
          {tasks.map((task) => <TaskTableRow handleFilterTasks={handleFilterTasks} key={task.id} task={task} showActive={showActive} />)}
        </TableBody>
      </Table>
    </TableContainer>
  );
