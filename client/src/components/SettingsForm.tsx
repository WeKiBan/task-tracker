...
export function SettingsForm({ onSave, saveLabel = "Save Changes" }: SettingsFormProps) {
  ...
  useEffect(() => {
    setJiraUrl(settings.jiraBaseUrl);
    setStartText(settings.emailStartText);
    setEndText(settings.emailEndText);
  }, [settings.jiraBaseUrl, settings.emailStartText, settings.emailEndText]);
      ...
      <div className="space-y-2">
        ...
        <div className="flex gap-2">
          {(["light", "dark", "system"] as const).map((nextTheme) => (
            <Button
              key={nextTheme}
              variant={theme === nextTheme ? "default" : "outline"}
              size="sm"
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-background/80 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-card border rounded-xl shadow-2xl p-6">
        ...
        <Button className="w-full" size="lg" onClick={() => setLocation("/settings")}>
          Get Started
        </Button>
      </div>
    </div>
